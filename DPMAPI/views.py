...
def centroids(request):
	...
	if request.method == 'POST':
		...
		kmeans = KMeans(n_clusters=int(n_clusters),  init='random', max_iter=int(iterate), tol=float(tolerance), random_state=int(random_state)).fit(data)
...
def predicted(request):
	if request.method == 'POST':
		...
		trainData = pd.read_csv("E:\\ShivSirProject\\Django\\DPM\\Files\\U1TrainFile.csv").replace(np.nan, 0)
